        ...
        [
          {
            ...
            link: 'https://github.com/SergeySalimov/nodejs-project/blob/master/nginx_conf/nodejs-project.conf',
            description: 'Это моя конфигурация для сервера ngnix. Запросы к express которые все начинаются с /api/ проксируются на порт на котором работает экспресс, а для Ангулар приложения сделано' +
              '2 варианта: 1-й когда оно отдается как статика из папки. Дя этого настроена прекомпрессия файлов и сами файлы Ангулара отдаются в сжатом виде. ' +
              '2-й вариант когда файлы Ангулара забираются с express, они сжимаются на лету и отдаются пользователю. Также настроено кэширование ответов. ' +
              'Логи сохраняются в отдельную папку, формат логов перенастроен.'
          },
          {
